...
uint8_t *do_decompress_rom(const uint8_t *comp, size_t comp_size, size_t *out_size) {
    ...
    for (int i = 0; i < dma_num; i++) {
        ...
        if (pend != 0) {
            /* Compressed file */
            uint32_t src_size = pend - pstart;
            uint32_t file_sz  = vend - vstart;
            if (ver->codec == CODEC_ZLIB) {
                size_t skip = ver->noheader ? 0 : 8;
                zlib_decode(comp, pstart + skip, src_size - skip,
                            dec, vstart, file_sz);
            } else {
                yaz0_decode(comp, pstart, src_size, dec, vstart);
            }
            decomp_count++;
        } else {
